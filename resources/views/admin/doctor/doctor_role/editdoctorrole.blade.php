    ...
    <div class="row">
        <div class="col-xxl-12">
            <div class="card shadow-sm">
                <div class="card-body">
                    <form method="POST" action="{{ route('doctor.updateRole', $role->id) }}" id="updateRole">
                        @csrf
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('name') is-invalid @enderror"
                                        id="name" name="name" value="{{ isset($role->name) ? $role->name : '' }}"
                                        placeholder="{{ __('main.Enter your username') }}" required>
                                </div>
                            </div>
                            <div class="col-lg-12">
                                <div class="row">
                                                        ...
                                                        <ul style="list-style-type: none; padding-inline-start: 0px;">
                                                            <li>
                                                                <div class="form-check mb-1">
                                                                    <input class="select_group form-check-input"
                                                                        type="checkbox"
                                                                        data-group-name="{{ preg_replace('/\s+/', '', $group) }}"
                                                                        {{ $group_check_status ? 'checked' : '' }}>
                                                                    <label class="form-check-label fw-semibold"
                                                                        for="formCheck2">{{ ucwords($group) }} </label>
                                                                </div>
                                                                        ...
                                                                        <li>
                                                                            @if (in_array($allPermissionsList->id, $permission['rolePermissions']))
                                                                                <div class="form-check mb-1">
                                                                                    <input type="checkbox"
                                                                                        name="permission[]"
                                                                                        id="md_checkbox_{{ $allPermissionsList->id }}"
                                                                                        class="form-check-input group_{{ preg_replace('/\s+/', '', $group) }}"
                                                                                        checked
                                                                                        value="{{ $allPermissionsList->name }}">
                                                                                    <label class="form-check-label"
                                                                                        for="md_checkbox_{{ $allPermissionsList->id }}">{{ $allPermissionsList->name }}
                                                                                    </label>
                                                                                </div>
                                                                            @else
                                                                                <div class="form-check mb-1">
